...
PcbPtr createnullPcb()
{
    ...
    new_process_Ptr->num_iter = -1;
    new_process_Ptr->level_no = LEVEL_DEFAULT;
    new_process_Ptr->memory_required = 0;
    new_process_Ptr->memory_ptr = NULL;
 ...
 * returns head of queue
 ******************************************************/
PcbPtr enqPcbHead(PcbPtr* q, PcbPtr p)
{
    if (!q)
    {
        return p;
