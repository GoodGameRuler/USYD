...
int main (int argc, char *argv[])
{
    ...
    PcbPtr arrive_jobs_queue = NULL;
    PcbPtr lv0_ready_queue = NULL;
    PcbPtr lv1_ready_queue = NULL;
    PcbPtr lv2_ready_queue = NULL;

    MabPtr root_memory = (MabPtr) malloc(sizeof(Mab));
    root_memory->size = 2048;
    root_memory->offset = 0;
    root_memory->parent = NULL;
    root_memory->left_child = NULL;
    root_memory->right_child = NULL;
    root_memory->allocated = 0;
    ...
    while (!feof(input_list_stream)) {  // put processes into fcfs_queue
        ...
        if (fscanf(input_list_stream,"%d, %d, %d",
                    ...
                    &(process->memory_required)) != 3) {
            free(process);
            continue;
        }

        if(process->memory_required < 8 || process->memory_required > 2048) {
            printf("Error: invalid mem requirement secified\n");
            exit(EXIT_FAILURE);
        }
    ...
    while (current_process || dispatched_process || arrive_jobs_queue || dispatch_queue || lv0_ready_queue || lv1_ready_queue || lv2_ready_queue)
    {
        if (current_process)
            //      i. If there is a currently running process;
        {
            ...
            if (current_process->remaining_cpu_time <= 0)
            {
                ...
                printf("Process %d-%d: terminated and freed %d memory\n", current_process->arrival_time, current_process->service_time, current_process->memory_ptr->size);
                memFree(current_process->memory_ptr);
                terminatePcb(current_process);
        ...
        while (!dispatched_process && dispatch_queue && dispatch_queue->arrival_time <= timer)
        {
            ...
            arrive_jobs_queue = enqPcb(arrive_jobs_queue, dispatched_process);
            printf("Process %d-%d: dispatched into arrive_jobs_queue ready queue at time %d\n", dispatched_process->arrival_time, dispatched_process->service_time, timer);
            dispatched_process = NULL;
        }

        int memory_available = 1;

        while (!dispatched_process && arrive_jobs_queue && memory_available)
        {
            //          Dequeue the process at the head of the queue, set it as currently running and start it
            MabPtr temp_ptr = memAlloc(root_memory, arrive_jobs_queue->memory_required);
            temp_ptr = memSplit(temp_ptr, arrive_jobs_queue->memory_required);

            if(temp_ptr) {
                arrive_jobs_queue->memory_ptr = temp_ptr;
                dispatched_process = deqPcb(&arrive_jobs_queue);
                lv0_ready_queue = enqPcb(lv0_ready_queue, dispatched_process);
                printf("Process %d-%d: allocated %d mem and placed into level 0 ready queue at time %d\n", dispatched_process->arrival_time, dispatched_process->service_time, temp_ptr->size, timer);
                dispatched_process = NULL;
            } else {
                memory_available = 0;
            }
