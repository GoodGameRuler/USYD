...
int main(int argc, char ** argv) {
	...
	threads = malloc((no_of_customers+1) * sizeof(pthread_t)); //total is No_Of_Consuers + 1 to include barber 
	if(threads == NULL){
		fprintf(stderr, "threads out of memory\n");
		exit(1);
	}	
	t_ids = malloc((no_of_customers+1) * sizeof(int)); //total is No_Of_Consuers + 1 to include barber
	if(t_ids == NULL){
	...
	{
		sleep((int)rand() % (customer_rate.max_pace - customer_rate.min_pace) + customer_rate.min_pace); //sleep a few second before creating a thread
		t_ids[k] = k;
...
void * customer_routine(void * arg) {
    ...
    if (no_of_chairs_occupied == no_of_chairs) {
		...
		do {
            ...
            ticket_to_be_called = last_ticket_called >= no_of_chairs ? 1 : last_ticket_called + 1;
            printf("Customer [%d]: Waiting for assistant to call %d, current call %d.\n", *customer_number, my_ticket, ticket_to_be_called);
